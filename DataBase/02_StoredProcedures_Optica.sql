...
CREATE PROCEDURE insertarCliente(	/* Datos Personales */
                                    ...
                                    IN	var_telmovil        VARCHAR(20),    -- 13
                                    IN	var_email           VARCHAR(129),    -- 14
                                    in	var_rfc				VARCHAR(13),	-- 15
                                    
                                    /* Valores de Retorno */
                                    OUT	var_idPersona       INT,            -- 16
                                    OUT	var_idCliente       INT,            -- 17
                                    OUT	var_numeroUnico     VARCHAR(65)     -- 18
				)                                    
    BEGIN        
        -- Comenzamos insertando los datos de la Persona:
        INSERT INTO persona (nombre, apellidoPaterno, apellidoMaterno, genero,
...
DROP PROCEDURE IF EXISTS insertarLente;
DELIMITER $$
CREATE PROCEDURE insertarLente(	/* Datos del Producto */
                                    IN	var_nombre          VARCHAR(64),    --  1
                                    IN	var_marca           VARCHAR(64),    --  2
                                    IN	var_precioCompra    DOUBLE,         --  3
                                    IN  var_precioVenta     DOUBLE,         --  4
                                    ...
                                    IN var_color			VARCHAR(45),	--  8
                                    
                                    
                                    /* Valores de Retorno */
                                    OUT	var_idProducto      INT,            -- 9
                                    OUT	var_idLenteContacto       INT,      -- 10
                                    OUT var_codigoBarras    VARCHAR(48)     -- 11
				)                                    
    BEGIN        
        -- Comenzamos insertando los datos del Producto:
        INSERT INTO producto (nombre, marca, precioCompra, precioVenta, existencias, estatus)
        ...
        -- Finalmente, insertamos en la tabla Lentes:
        INSERT INTO lente_contacto (idProducto, keratometria, fotografia, color)
                    VALUES(var_idProducto, kerato, var_fotografia, var_color);
        -- Obtenemos el ID del Armazon que se genero:
...
DROP PROCEDURE IF EXISTS actualizarLente;
DELIMITER $$
CREATE PROCEDURE actualizarLente(	/* Datos del Producto */
                                    IN  var_codigoBarras    VARCHAR(48),    --  1
                                    IN	var_nombre          VARCHAR(64),    --  2
                                    IN	var_marca           VARCHAR(64),    --  3
                                    IN	var_precioCompra    DOUBLE,         --  4
                                    ...
                                    IN  var_color			VARCHAR(45),	--  9
                                    IN	var_idProducto      INT,            --  10     
                                    
                                    IN	var_idLenteContacto       INT       -- 11
				)                                    
    BEGIN        
        -- Actualizamos los datos del Producto:
        UPDATE producto SET codigoBarras = var_codigoBarras, nombre = var_nombre, marca = var_marca, 
                            ...
                            existencias = var_existencias
                        WHERE idProducto = var_idProducto;
		-- Actualizamos los datos del Lente:
		UPDATE lente_contacto SET keratometria = kerato, fotografia = var_fotografia, color = var_color                            
...
DROP TRIGGER IF EXISTS trg_venta_producto;
DELIMITER $$

CREATE TRIGGER trg_venta_producto AFTER INSERT ON venta_producto
FOR EACH ROW
BEGIN
